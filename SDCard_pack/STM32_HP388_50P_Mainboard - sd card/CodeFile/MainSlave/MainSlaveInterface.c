...
void Speak(u8 SpeakCount)//蜂鸣器
{
    u32 i = 0,Times;

	for(Times = 0; Times < SpeakCount ;Times++)
	{
    for (i = 0; i < 100; i++)
    {
...
void SetHighVoltage(void)	//电源板选择高电压输出
{
		GPIO_ResetBits(GPIOB, GPIO_Pin_5);
		GPIO_SetBits(GPIOB, GPIO_Pin_8);
}

void SetLowVoltage(void)	//电源板选择低电压输出
{
...
{
    int i;
		u8 TmpBuf[0x110];  //发送数据数组
    u16 Crc16;
    //u16 DataLenTmp;
	
    memset_data(TmpBuf, 0, 0x110);        //清空发送数组
		
    //--------配置传输格式--------//

    //-------- 4Byte命令位 + 2Byte长度位(命令位+长度位+数据总长度,
