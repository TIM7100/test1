...
//#include "AliyunIOT.h"              //包含需要的头文件
//#include "ethernet.h"               //包含需要的头文件
...
//rb_t rb_t_ssl;                         //存储环形队列信息的结构体
//char rb_t_ssl_buff[rb_t_ssl_buff_len]; //存储数据的环形队列Buff
...
static int CustomSSLRecv(void *ctx, unsigned char *RxBuf, size_t Len)
{
    ...
    else
    {
        ...
        if (rbCanReadLen > 0)
        {
            ...
            {
                //只返回当前有的
                rbRead(&rb_t_ssl, RxBuf, rbCanReadLen);
                return rbCanReadLen;
            }
            else
            {
                //返回足够的数据
                rbRead(&rb_t_ssl, RxBuf, Len);
                return Len;
            }
        }
        else
...
static int CustomSSLSend(void *ctx, const unsigned char *TxBuf, size_t Len)
{
    ...
    {
        UartSend_bytes(UART3, (u8*)TxBuf, Len);
    }
