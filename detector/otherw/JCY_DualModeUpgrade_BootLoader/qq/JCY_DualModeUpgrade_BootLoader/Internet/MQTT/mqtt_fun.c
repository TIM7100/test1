...
u16 WaitReceiveMsg(MQTT_PACKET_INFOR *MqttPacket)
{
	...
	{
		Len = rbCanRead(&rb_t_ssl);
	}
	
	if(Len > 0)
	{
		memset(MqttPacket->DataPacket, 0, MqttPacket->DataPacketLen);
        Len = SSL_ReadData(MqttPacket->DataPacket, MqttPacket->DataPacketLen);//recv(SOCK_TCPS, MqttPacket->DataPacket, MqttPacket->DataPacketLen);                 //接收物联网发布的消息
//		PrintfRxBuff(MqttPacket->DataPacket, Len);
	}
	return Len;
}
...
/* 拼接连接报文格式，并通过W5500发送拼接好的报文, 云平台服务器接收到连接报文后，会返回CONNACK回应，通过判断接收的回应的值，来确定是否连接成功 */
	...
	if(NetWork_Flag == W5500)
	{
		...
		if (Ret != 0)    
		{
			//recv(SOCK_TCPS, MqttPacket->DataPacket, MqttPacket->DataPacketLen);
...
//	PrintfHex(MqttPacket->DataPacket, PacketLen);	      	           //打印连接报文
	//send(SOCK_TCPS, MqttPacket->DataPacket, PacketLen);                    //发送链接云平台设备请求
	...
	{
		rbCanReadLen = rbCanRead(&rb_t_ssl);							   //检测云平台有没有返回回应
		while (rbCanReadLen == 0 && Timeout--)   
		{
			//printf("esp8266 wait connect ack\r\n");
			delay_ms(500);
			if (Timeout == 0)
			{
				return CONNECT_MQTT_CLIENT_ERR;
	...
	PacketLen = SSL_ReadData(MqttPacket->DataPacket, MqttPacket->DataPacketLen); //recv(SOCK_TCPS, MqttPacket->DataPacket, MqttPacket->DataPacketLen);        //接收云平台返回状态
//	PrintfHex(MqttPacket->DataPacket, PacketLen);									   //打印接收到的回应
...
//		PrintfRxBuff(MqttPacket->DataPacket, PacketLen);	      	          //打印订阅报文
		//send(SOCK_TCPS, MqttPacket->DataPacket, PacketLen);                   //发送订阅报文
		...
		PacketLen = SSL_ReadData(MqttPacket->DataPacket, MqttPacket->DataPacketLen);//recv(SOCK_TCPS, MqttPacket->DataPacket, MqttPacket->DataPacketLen);       //接收云平台返回状态				                  //打印接收到的回应
...
/* 发布消息的服务质量等级 QoS 若为1，则服务器接收发布报文后，会返回PUBACK回应，若QoS为0，则不会有回应 */
	...
	//			printf("wait pub ack\r\n");
		...
		PacketLen = SSL_ReadData(MqttPacket->DataPacket, MqttPacket->DataPacketLen);          //recv(SOCK_TCPS, MqttPacket->DataPacket, MqttPacket->DataPacketLen);           //接收云平台服务器返回来的状态信息
		//PrintfHex(MqttPacket->DataPacket, PacketLen);					                      //打印接收到的回应
		Ret = MqttAckMsg(MqttPacket->DataPacket);							                  //解析返回的回应是否为PUBACK
		if (Ret != PUBACK)
		{
			printfS("puback err\r\n");
       //     PrintfHex(MqttPacket->DataPacket, 4);
			return PUB_TOPIC_ERR;
		}
	}
	return PACKET_SUCCESS;
...
//	PrintfHex(KeepBuf, PacketLen);	      	                             //打印心跳报文
	//send(SOCK_TCPS, KeepBuf, PacketLen);                                     //发送心跳报文
	...
	{
		Ret = rbCanRead(&rb_t_ssl);									 		//检测云平台有没有返回回应
		while (Ret == 0 && Timeout--)    
		{
			printf("wait ping ack\r\n");
			System_Delay_MS(10);
			if (Timeout == 0)
			{
				return KEEP_ALIVE_ERR;
	...
	PacketLen = SSL_ReadData(KeepBuf, sizeof(KeepBuf)); //recv(SOCK_TCPS, KeepBuf, sizeof(KeepBuf));                          //接收云平台返回状态
//	PrintfHex(KeepBuf, PacketLen);                                 				//打印接收到的回应
