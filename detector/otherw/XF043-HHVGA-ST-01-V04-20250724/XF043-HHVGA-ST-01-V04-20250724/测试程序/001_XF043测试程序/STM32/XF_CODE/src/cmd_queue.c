...
*/
qsize queue_find_cmd(qdata *buffer,qsize buf_len,qdata cmd_head)
{
    qsize cmd_size = 0;
    qdata _data = 0;
#if IS_CRC16  
    int crc_pos=1;  //crc起始位置
#endif
    while(queue_size()>0)
    {
        //取一个数据
...
//SendChar(_data);  //调试数据
  ...
  #if 0   //调试        
  for(i=crc_pos;i<=cmd_size-crc_pos-4;i++) //调试数据
  {
   SendChar(buffer[i]);
  }
  #endif         
            ...
            //去掉指令头EE，尾FFFCFFFF，地址，只计算数据部分CRC
          if(!CheckCRC16(buffer+crc_pos,cmd_size-crc_pos-4))  //CRC校验
              return 0;
