...
void ShowMenu(u8 Language)
{
    ...
    if (ScreenRefresh != 0)
    {
        ...
        UartSend("DC24(114,40,'Please select type',15);\r\n");
        CheckBusy();
        ...
        for (Display_count = DisplayStart; Display_count < DisplayEnd; Display_count++)
        {
            ...
            UartSend(CMD_DC24(160, 60 + (n * 36), MenuPoint[Display_count].DisplayString[Language], color));
            CheckBusy();
            n++;
        }
    }
    else
    {
        ...
        UartSend(CMD_DC24(160, 60 + (last_cow * 36), MenuPoint[DisplayStart + last_cow].DisplayString[Language], 0));
        CheckBusy();
        UartSend(CMD_DC24(160, 60 + (last_cow * 36), MenuPoint[DisplayStart + last_cow].DisplayString[Language], 4));
        CheckBusy();

        now_cow = Select_Point - 1;
        UartSend(CMD_DC24(160, 60 + (now_cow * 36), MenuPoint[DisplayStart + now_cow].DisplayString[Language], 0));
        CheckBusy();
        UartSend(CMD_DC24(160, 60 + (now_cow * 36), MenuPoint[DisplayStart + now_cow].DisplayString[Language], 6));
        CheckBusy();
    }
    LayerSwitchFlag = 0;
