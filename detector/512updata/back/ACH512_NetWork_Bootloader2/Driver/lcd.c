...
char* CMD_DC24(u16 x, u16 y, u8 *str, u8 Color)
{
    ...
    for (i = 0; i < 5; i++)
    {
        Cmd_Buf[i] = Cmd_Start[i];
    }

    Set_X = x;
    Cmd_Buf[i++] = Set_X / 100 + 0x30;
    Cmd_Buf[i++] = Set_X % 100 / 10 + 0x30;
    Cmd_Buf[i++] = Set_X % 10 + 0x30;
    Cmd_Buf[i++] = ',';

    Set_Y = y;
    Cmd_Buf[i++] = Set_Y / 100 + 0x30;
    Cmd_Buf[i++] = Set_Y % 100 / 10 + 0x30;
    Cmd_Buf[i++] = Set_Y % 10 + 0x30;
    Cmd_Buf[i++] = ',';
    Cmd_Buf[i++] = '\'';
