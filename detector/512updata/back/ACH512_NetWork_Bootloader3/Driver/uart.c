...
//             rx_count++;
//             rx_flag = 1;
// 					 byte = (u8)REG_UART_DR(UARTB);
//             rbWriteByte(&rb_t_ssl, byte);   /* 数据真正进入 rb_t_ssl */
//         }
//     }
//     else if (temp & 0x20) // Tx int
//     {
...
//     if (temp & 0x10)      // Rx 中断
//     {
//         /* 把 FIFO 中所有可用字节读出并写入环形缓冲 */
//         while ((REG_UART_FR(UARTB) & 0x10) != 0x10)  // 直到 Rx FIFO 空
//         {
//             byte = (u8)REG_UART_DR(UARTB);
//             rbWriteByte(&rb_t_ssl, byte);   /* 数据真正进入 rb_t_ssl */
//         }
//     }
//     else if (temp & 0x20) // Tx 中断
//     {
//         if ((REG_UART_FR(UARTB) & 0x80) == 0x80)  // Tx FIFO 空
//         {
//             tx_flag = 1;
//         }
//     }
