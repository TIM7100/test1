 ...
 *                                  数据上报云平台的模板                                         *
************************************************************************************************/
#define THING_UPLOAD_TEMPLATE             "{\"method\":\"report\",\"clientToken\":\"123\",\"timestamp\":1628646783,\"params\":{\"DV\":\"%s\",\"SJY_COUNT\":%d}}"
...
typedef struct SHADOW_INFO{
    u32 Lock;           //不是45464546，会初始化内部flash点数存储区一次
	u32 ShadowCount;    //点数
	u32 Version;		//影子设备版本
	u8 CountValidFlag;  //1:APP点数未获取，0:APP点数已获取
}SHADOW_INFO_t;   
/***********************************************************************************************
 *                                    函数返回值的枚举集合                                       *
************************************************************************************************/
enum {
	...
	OTA_DONT_UPDATA,	                                 //12
	OTA_COMMAND_ADD,	                                 //13
	OTA_COMMAND_UPDATA,	                               //14
	OTA_COMMAND_ANALYSIS,	                             //15
	OTA_ADD_SERIES_ZERO,	                             //16
	OTA_HTTP_SUCCESS,	                                 //17
	OTA_CONNECT_TIMEOUT,	                             //18
	OTA_COMMAND_DELETE_UPDATE,	                       //19
	OTA_DOWNLOAD_ONCE_SUCCES,	                         //20
	OTA_FIRMWARE_VERIFY_ERROR,	                       //21
	OTA_FIRMWARE_VERIFY_SUCCES,	                       //22
	OTA_LINK_CLOUD_ERROR,	                             //23
	OTA_USER_CANCEL,	                                 //24
	OTA_FIRMWARE_EMPTY,	                               //25
	OTA_JUMP_APP_ERROR,	                               //26
	OTA_WRITE_FLASH_ERROR,	                           //27
	OTA_DEVICE_ABNORMAL,	                             //28
	SHADOW_SET_SUCCESS,	                               //29
	SHADOW_VERSION_ERROR,	                             //30
};
...
u8 UploadFirmwareVersion_Res(void);
#endif
