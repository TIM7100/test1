...
extern uint8_t WiFi_Connect_IoTServer(char *WiFiServerIP, uint16_t WiFiServerPort, int16_t timeout);
...
u8 FactoryReset(void)
{
    ...
    Send_SetText(0x0001, InitING, sizeof(InitING));
    Send_SetText(0x0004, InitING_en, sizeof(InitING_en));

   


    ConnectShow();
    if (UploadFirmwareVersion_Res() != OTA_SUCCESS)
    {
			ConnectTencentCloudShow();
...
//                 ProgressBarShow(HttpPacketInfo.DownloadCount, HttpPacketInfo.DownloadNum + 1);                      //加载进度条
                percent = (u8)(((u32)(HttpPacketInfo.DownloadNum + 1) * 100UL) / HttpPacketInfo.DownloadCount);
                Send_SetCtrVAL(2, percent);
            }
        }
