        ...
        case 0x05:  //获取控件的文本内容
            ...
            case 0x0004:
                ...
                if (memcmp(UserPassword, Resetdevice, 4) == 0)
                {
                    ...
                    if (status == 0)
                    {
                        Send_SetText(0x0001, textInitOk, sizeof(textInitOk));        //设备初始化成功
                        Timer0DelayMs(30);
                        Send_SetText(0x0004, textInitOk_en, sizeof(textInitOk_en));
                    }
                    else
                    {
                        Send_SetText(0x0001, textNetFailed, sizeof(textNetFailed));
                        Timer0DelayMs(30);
                        Send_SetText(0x0004, textNetFailed_en, sizeof(textNetFailed_en));
                    }
