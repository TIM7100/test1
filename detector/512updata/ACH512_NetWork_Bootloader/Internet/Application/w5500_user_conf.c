...
wiz_NetInfo WizNetInfo;			 //存放了配置W5500网络所需要的参数
/***********************************************************************************************
 *                                     静态函数声明区                                           *
************************************************************************************************/
...
void DhcpInit(void)
{
	static u8 DHCP_Buff[1024];                //DHCPBuff 
...
{
    REG_GPIO_CLR(GPIOA) |= (1 << 19);     //拉低硬件复位引脚
    Timer0DelayMs(2);                                             //延时，至少500us以上低电平
    REG_GPIO_SET(GPIOA) |= (1 << 19);      //拉高硬件复位引脚
    Timer0DelayMs(1800);                                           //延时
}
...
{
    spi_init(SPI_W5500, WORK_MODE_3);
	CsDeselect();			//释放片选脚
    /*定义RESET引脚*/
	REG_SCU_MUXCTRLB &= ~(0x03 << 6);    //设置管脚复用为GPIO19 res
	REG_SCU_PUCRA |= (1 << 19);			 //res上拉
	REG_GPIO_DIR(GPIOA) |= (1 << 19);    //RES为输出 
	/*定义INT引脚*/
	REG_SCU_MUXCTRLB &= ~(0x03 << 4);    //设置管脚复用为GPIO18 res	
	REG_SCU_PUCRA |= (1 << 18);			 //res上拉	
	REG_GPIO_DIR(GPIOA) |= (1 << 18);    //RES为输出 
