...
//     }
    ...
    RTC_NVIC_Config();//RCT中断分组设置
		if (MainCount_Hex == 100)
    {
        RTC_Set(2023, 7, 31, 15, 14, 30); //设置时间
        MainCount_Hex += 10;
    }
    RTC_Get();//更新时间
    return 0; //ok
...
u8 RTC_Alarm_Set(u16 syear, u8 smon, u8 sday, u8 hour, u8 min, u8 sec)
{
    ...
    for (t = 1970; t < syear; t++) //把所有年份的 秒钟相加
    {
        if (Is_Leap_Year(t))
        {
            seccount += 31622400;    //闰年的秒钟数
...
//     tmp = calendar.w_date;
//     tmp = calendar.w_date % 100;
//     time_arr[4] = tmp / 10  + 0x30;
//     tmp = calendar.w_date % 10;
//     time_arr[5] = tmp  + 0x30;

//     tmp = calendar.hour;
//     tmp = calendar.hour % 100;
//     time_arr[6] = tmp / 10  + 0x30;
//     tmp = calendar.hour % 10;
//     time_arr[7] = tmp  + 0x30;
...
//     tmp = calendar.sec;
//     tmp = calendar.sec % 100;
//     time_arr[10] = tmp / 10  + 0x30;
//     tmp = calendar.sec % 10;
//     time_arr[11] = tmp  + 0x30;
// //     time_arr[12] = 0;
// }
