...
void Get_TableHead(void)
{
    ...
    W25QXX_Read(buffer, 0x00, 2 * 14); //读取Flash数据的表头，每个型号14字节

    for (i = 0; i < 2; i ++)
    {
        for (j = 0; j < 14; j++)
        {
            *((u8 *)&FlashTable + j + (14 * i)) =   buffer[j + (i * 14)];
        }
    }
}
...
u8 Get_SN_number(TypeInfrom TypeSel, u32 SN_Count, u8 ReadNum) //读取flash中的序列号 ，12位结构体， 大板烧录总数， 读取个数
{
    ...
    StartAdddr = (TypeSel.FlashStartAddr[0] << 24) + (TypeSel.FlashStartAddr[1] << 16) + (TypeSel.FlashStartAddr[2] << 8) + (TypeSel.FlashStartAddr[3]); //序列号开始地址
    ...
    if (SNTotal > 0)
    {
        for (i = 0; i < ReadNum; i++)
        {
            W25QXX_Read((u8*)&Receive_SN[0], (RealCount * 0xA6) + StartAdddr, 0xA6); //读取需要个数的序列号存于2维数组Receive_SN中
...
u8 rx1108A[4096];
u8 tx1108A[28] = {0x00, 0x00, 0x00, 0xA6, 0x08, 0x00, 0x00, 0x17, 0x00, 0x00, 0x00, 0x00,
									 	 0x00, 0x01, 0x00, 0xA6, 0x1E, 0xB2, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00,
};
void SPI_TEST_Burn(void)
{
    W25QXX_Write(tx1108A, 0x00, 28);
   
    W25QXX_Read(rx1108A, 0x00, 4096);
